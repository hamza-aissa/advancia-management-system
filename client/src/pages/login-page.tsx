...
export function LoginPage() {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10"><div className="w-full max-w-sm"><div className="mb-5 text-center"><div className="mx-auto mb-3 grid size-9 place-items-center rounded-md bg-primary text-primary-foreground"><LockKeyhole className="size-4" /></div><h1 className="text-xl font-semibold">Advancia</h1><p className="mt-1 text-sm text-muted-foreground">Accès à l’espace de gestion interne</p></div><Card><CardHeader className="pb-4"><CardTitle className="text-base">Connexion</CardTitle><CardDescription>Utilisez votre compte professionnel.</CardDescription></CardHeader><CardContent><form onSubmit={handleSubmit(submit)} className="space-y-4" noValidate>{serverError && <div role="alert" className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800">{serverError}</div>}<div className="space-y-1.5"><Label htmlFor="email">Adresse e-mail</Label><Input id="email" type="email" autoComplete="email" autoFocus placeholder="nom@advancia.com" aria-invalid={Boolean(errors.email)} {...register('email')} />{errors.email && <p className="text-xs text-red-600">{errors.email.message}</p>}</div><div className="space-y-1.5"><Label htmlFor="password">Mot de passe</Label><Input id="password" type="password" autoComplete="current-password" aria-invalid={Boolean(errors.password)} {...register('password')} />{errors.password && <p className="text-xs text-red-600">{errors.password.message}</p>}</div><Button className="w-full" disabled={isSubmitting || isLoading}>{isSubmitting ? <><LoaderCircle className="animate-spin" />Connexion…</> : 'Se connecter'}</Button></form><div className="mt-5 border-t pt-4"><p className="mb-2 text-[11px] font-medium text-muted-foreground">Comptes de démonstration · mot de passe : password123</p><div className="grid gap-1.5">{demoUsers.map((demo) => <button key={demo.role} type="button" onClick={() => chooseDemo(demo.email)} className="flex h-8 items-center justify-between rounded-md border bg-slate-50 px-2.5 text-left text-xs hover:bg-slate-100"><span className="font-medium">{demo.role}</span><span className="text-muted-foreground">{demo.email}</span></button>)}</div></div></CardContent></Card></div></main>
}
