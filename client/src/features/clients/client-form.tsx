...
const requiredSchema = z.object({
  ...
  assignedConsultant: z.string().min(1, 'Select a consultant'),
...
const selectClass = 'flex h-11 w-full rounded-md border border-input bg-card px-3 py-2 text-sm shadow-sm outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/15'
...
function FullClientForm({ mode, client, agents = [], consultants = [], isSubmitting, serverErrors = {}, submitLabel, onCancel, onSubmit }: ClientFormProps) {
  ...
  return <form className="space-y-5 p-6" onSubmit={form.handleSubmit((values) => onSubmit({ ...values, phone: values.phone || undefined, address: values.address || undefined, notes: values.notes || undefined, lastContactAt: values.lastContactAt ? new Date(values.lastContactAt).toISOString() : null }))} noValidate>
    <div className="grid gap-5 sm:grid-cols-2">
      <div className="space-y-2"><Label htmlFor="client-name">Client name</Label><Input id="client-name" autoComplete="organization" {...form.register('name')} aria-invalid={Boolean(field('name'))} aria-describedby={field('name') ? 'client-name-error' : undefined} /><FieldError id="client-name-error" message={field('name')} /></div>
      ...
      <div className="space-y-2"><Label htmlFor="client-phone">Phone</Label><Input id="client-phone" autoComplete="tel" {...form.register('phone')} aria-invalid={Boolean(field('phone'))} aria-describedby={field('phone') ? 'client-phone-error' : undefined} /><FieldError id="client-phone-error" message={field('phone')} /></div>
      <div className="space-y-2"><Label htmlFor="client-contact">Latest contact</Label><Input id="client-contact" type="datetime-local" {...form.register('lastContactAt')} aria-invalid={Boolean(field('lastContactAt'))} aria-describedby={field('lastContactAt') ? 'client-contact-error' : undefined} /><FieldError id="client-contact-error" message={field('lastContactAt')} /></div>
      <div className="space-y-2"><Label htmlFor="assigned-agent">Assigned agent</Label><select id="assigned-agent" className={selectClass} {...form.register('assignedAgent')} aria-invalid={Boolean(field('assignedAgent'))} aria-describedby={field('assignedAgent') ? 'assigned-agent-error' : undefined}><option value="">Select an agent</option>{agents.map((user) => <option key={user.id} value={user.id}>{user.firstName} {user.lastName}</option>)}</select><FieldError id="assigned-agent-error" message={field('assignedAgent')} /></div>
      <div className="space-y-2"><Label htmlFor="assigned-consultant">Assigned consultant</Label><select id="assigned-consultant" className={selectClass} {...form.register('assignedConsultant')} aria-invalid={Boolean(field('assignedConsultant'))} aria-describedby={field('assignedConsultant') ? 'assigned-consultant-error' : undefined}><option value="">Select a consultant</option>{consultants.map((user) => <option key={user.id} value={user.id}>{user.firstName} {user.lastName}</option>)}</select><FieldError id="assigned-consultant-error" message={field('assignedConsultant')} /></div>
    </div>
    <div className="space-y-2"><Label htmlFor="client-address">Address</Label><Input id="client-address" autoComplete="street-address" {...form.register('address')} aria-invalid={Boolean(field('address'))} aria-describedby={field('address') ? 'client-address-error' : undefined} /><FieldError id="client-address-error" message={field('address')} /></div>
    <div className="space-y-2"><Label htmlFor="client-notes">Operational notes</Label><Textarea id="client-notes" placeholder="Context the assigned team needs for the next contact" {...form.register('notes')} aria-invalid={Boolean(field('notes'))} aria-describedby={field('notes') ? 'client-notes-error' : undefined} /><FieldError id="client-notes-error" message={field('notes')} /></div>
    <div className="flex justify-end gap-3 border-t pt-5"><Button type="button" variant="outline" onClick={onCancel}>Cancel</Button><Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Saving…' : submitLabel || (mode === 'create' ? 'Create client' : 'Save changes')}</Button></div>
  </form>
}
...
function OperationalForm({ client, isSubmitting, serverErrors = {}, onCancel, onSubmit }: ClientFormProps) {
  ...
  return <form className="space-y-5 p-6" onSubmit={form.handleSubmit((values) => onSubmit({ notes: values.notes || '', lastContactAt: values.lastContactAt ? new Date(values.lastContactAt).toISOString() : null }))} noValidate>
    <div className="space-y-2"><Label htmlFor="operational-contact">Latest contact</Label><Input id="operational-contact" type="datetime-local" {...form.register('lastContactAt')} aria-invalid={Boolean(form.formState.errors.lastContactAt || serverErrors.lastContactAt)} aria-describedby={(form.formState.errors.lastContactAt || serverErrors.lastContactAt) ? 'operational-contact-error' : undefined} /><FieldError id="operational-contact-error" message={form.formState.errors.lastContactAt?.message || serverErrors.lastContactAt} /></div>
    <div className="space-y-2"><Label htmlFor="operational-notes">Operational notes</Label><Textarea id="operational-notes" placeholder="Record the latest client context and next conversation" {...form.register('notes')} aria-invalid={Boolean(form.formState.errors.notes || serverErrors.notes)} aria-describedby={(form.formState.errors.notes || serverErrors.notes) ? 'operational-notes-error' : undefined} /><FieldError id="operational-notes-error" message={form.formState.errors.notes?.message || serverErrors.notes} /></div>
    <p className="text-xs text-muted-foreground">Account details and assignments can only be changed by an administrator.</p>
    <div className="flex justify-end gap-3 border-t pt-5"><Button type="button" variant="outline" onClick={onCancel}>Cancel</Button><Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Saving…' : 'Save update'}</Button></div>
  </form>
}
