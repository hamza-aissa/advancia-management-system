...
export function ClientsPage() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h1 className="text-xl font-semibold">Clients</h1><p className="mt-1 text-sm text-muted-foreground">Consultez les comptes affectés et maintenez leur contexte à jour.</p></div>{isAdmin && <Button size="sm" onClick={() => { setServerErrors({}); setCreateOpen(true) }}><Plus />Nouveau client</Button>}</div>
    ...
    <Dialog open={createOpen} onOpenChange={setCreateOpen} title="Créer un client" description="Affectez un responsable de chaque service dès la création.">
      {(agentsQuery.isPending || consultantsQuery.isPending) ? <PageLoader label="Chargement de l’équipe" /> : (agentsQuery.isError || consultantsQuery.isError) ? <div className="p-6"><ErrorState message="Impossible de charger les collaborateurs disponibles." /></div> : <ClientForm mode="create" agents={agentsQuery.data} consultants={consultantsQuery.data} isSubmitting={createMutation.isPending} serverErrors={serverErrors} onCancel={() => setCreateOpen(false)} onSubmit={(input) => createMutation.mutate(input as ClientInput)} />}
