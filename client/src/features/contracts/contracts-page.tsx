...
export function ContractsPage() {
  ...
  const [createOpen, setCreateOpen] = useState(isConsultant && searchParams.get('nouveau') === '1')
  ...
  if (contracts.isLoading || (isConsultant && (clients.isLoading || contractTypes.isLoading))) return <PageLoader label="Chargement des contrats" />
  if (contracts.isError || (isConsultant && (clients.isError || contractTypes.isError))) return <ErrorState message={getApiErrorMessage(contracts.error || clients.error || contractTypes.error)} retry={() => { contracts.refetch(); clients.refetch(); contractTypes.refetch() }} />
  ...
  return <div className="space-y-6">
    <header className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
      ...
      {isConsultant && <Button size="sm" onClick={() => setCreateOpen(true)}><Plus />Nouveau contrat</Button>}
    </header>
    ...
    {rows.length === 0 ? <EmptyState title="Aucun contrat" message={contracts.data?.length ? 'Modifiez les filtres pour afficher d’autres contrats.' : 'Aucun contrat à afficher.'} action={isConsultant && !contracts.data?.length ? <Button onClick={() => setCreateOpen(true)}>Nouveau contrat</Button> : undefined} /> : <>
    ...
    {isConsultant && <ContractFormDialog open={createOpen} onOpenChange={setCreateOpen} initialClient={searchParams.get('client') || ''} clients={clients.data || []} contractTypes={contractTypes.data || []} busy={createContract.isPending} error={createContract.isError ? getApiErrorMessage(createContract.error) : undefined} onSubmit={async (values) => { await createContract.mutateAsync(values) }} />}
  </div>
}
