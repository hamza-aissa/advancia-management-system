...
export function ContractDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    <header className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setDialog('edit')}><Pencil />Modifier</Button>{isAdmin && <Button variant="outline" onClick={() => setDialog('assign')}><UserRoundCog />Réattribuer</Button>}</div>
    </header>

    <section aria-label="Actions de renouvellement" className="flex flex-wrap gap-2 rounded-lg border bg-card p-3"><Button onClick={() => setDialog('contact')}><PhoneCall />Marquer contacté</Button><Button variant="outline" onClick={() => setDialog('follow-up')}><CalendarClock />Planifier une relance</Button><Button variant="outline" onClick={() => setDialog('renew')}><RefreshCw />Renouveler</Button><Button variant="outline" className="text-red-700 hover:text-red-800" onClick={() => setDialog('decline')}><XCircle />Refuser</Button></section>

    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,1fr)]">
      <div className="space-y-6">
        <Card><CardHeader><CardTitle>Détails du contrat</CardTitle></CardHeader><CardContent><dl className="grid gap-4 sm:grid-cols-2"><Detail label="Client" value={clientName(item.client)} /><Detail label="Consultant responsable" value={personName(item.owner)} /><Detail label="Date de début" value={formatDate(item.startDate)} /><Detail label="Date d’échéance" value={formatDate(item.expiryDate)} /><Detail label="Valeur du contrat" value={formatMoney(item.value)} /><Detail label="Prochaine relance" value={formatDate(item.nextFollowUpAt)} /></dl>{item.description && <div className="mt-5 border-t pt-4"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Description</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{item.description}</p></div>}{item.declineReason && <div className="mt-5 rounded-md bg-red-50 p-3"><p className="text-xs font-semibold uppercase tracking-wide text-red-700">Motif du refus</p><p className="mt-1 text-sm text-red-900">{item.declineReason}</p></div>}</CardContent></Card>
