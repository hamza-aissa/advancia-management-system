...
const formatMoney = (value?: number) => value == null ? '—' : `${new Intl.NumberFormat('fr-TN', { minimumFractionDigits: 3, maximumFractionDigits: 3 }).format(value)} TND`

export function LicenseDetailPage() {
  const { id = '' } = useParams(); const { user } = useAuth(); const isAgent = user?.role === 'agent'; const queryClient = useQueryClient(); const [action, setAction] = useState<LicenseAction | null>(null)
  ...
  return <div className="space-y-6"><Link to="/licenses" className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />Retour aux licences</Link>
    ...
    {isAgent && <LicenseActionDialog action={action} busy={mutate.isPending} error={mutate.isError ? getApiErrorMessage(mutate.error, 'Impossible d’appliquer cette action.') : undefined} onClose={() => { setAction(null); mutate.reset() }} onSubmit={(values) => action && mutate.mutate({ kind: action, values })} />}
  </div>
}
