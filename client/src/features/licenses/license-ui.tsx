...
export function UrgencyBadge({ urgency, days }: { urgency: Urgency; days?: number }) {
  const suffix = days === undefined ? '' : days < 0 ? ` · ${Math.abs(days)} j de retard` : ` · ${days} j`
  return <span className={cn('inline-flex rounded-md border px-2 py-0.5 text-[11px] font-medium', urgencyStyles[urgency])}>{urgencyLabels[urgency]}{suffix}</span>
}

export function StatusBadge({ status }: { status: RenewalStatus }) {
  return <span className="inline-flex rounded-md border bg-white px-2 py-0.5 text-[11px] font-medium text-slate-700">{statusLabels[status]}</span>
}
...
export function Modal({ open, title, description, onClose, children }: { open: boolean; title: string; description?: string; onClose: () => void; children: ReactNode }) {
  if (!open) return null
  return <div className="fixed inset-0 z-50 flex items-stretch justify-end bg-slate-950/35" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section role="dialog" aria-modal="true" aria-labelledby="license-dialog-title" className="h-full w-full overflow-y-auto border-l bg-white p-4 sm:max-w-lg">
      <div className="flex items-start justify-between gap-4">
        ...
        <Button type="button" size="icon" variant="ghost" onClick={onClose} aria-label="Fermer"><X /></Button>
      </div>
...
export const fieldClass = 'h-8 w-full rounded-md border border-input bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-ring disabled:bg-muted'
...
export function formatDate(value?: string) {
  if (!value) return '—'
  return new Intl.DateTimeFormat('fr-TN', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(value))
}

export function personName(person?: LicensePerson) {
  return person ? `${person.firstName} ${person.lastName}`.trim() || person.email : 'Non attribué'
}
