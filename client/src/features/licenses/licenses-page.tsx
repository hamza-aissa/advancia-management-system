...
export function LicensesPage() {
  ...
  if (licenses.isLoading || clients.isLoading || (isAdmin && agents.isLoading)) return <PageLoader label="Chargement des licences" />
  ...
  return <div className="space-y-6">
    ...
    <LicenseFormDialog open={dialogOpen} license={editing} clients={clients.data || []} agents={agents.data || []} isAdmin={isAdmin} busy={save.isPending} error={save.isError ? getApiErrorMessage(save.error, 'Impossible d’enregistrer la licence.') : undefined} onClose={() => setDialogOpen(false)} onSubmit={(input) => save.mutate(input)} />
  </div>
}
