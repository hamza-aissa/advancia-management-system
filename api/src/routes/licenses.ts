...
// Agents manage licences; admins supervise, reassign and archive.
router.post('/', writeLimiter, authorize(UserRole.AGENT), validate(licenseCreateSchema), createLicense);
router.get('/', authorize(UserRole.AGENT, UserRole.ADMIN), getLicenses);
router.post('/:id/contact', writeLimiter, authorize(UserRole.AGENT), validate(idParamsSchema, 'params'), validate(noteSchema), markLicenseContacted);
router.post('/:id/follow-up', writeLimiter, authorize(UserRole.AGENT), validate(idParamsSchema, 'params'), validate(followUpSchema), scheduleLicenseFollowUp);
router.post('/:id/renew', writeLimiter, authorize(UserRole.AGENT), validate(idParamsSchema, 'params'), validate(renewSchema), renewLicense);
router.post('/:id/decline', writeLimiter, authorize(UserRole.AGENT), validate(idParamsSchema, 'params'), validate(declineSchema), declineLicense);
